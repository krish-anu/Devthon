...
describe('RecaptchaService', () => {
  ...
  it('skips verification when RECAPTCHA_SECRET is not set', async () => {
    ...
    const svc = new RecaptchaService(mockConfig as any);
    const ok = await svc.verify(null);
    expect(ok).toBe(true);
  });

  it('returns false when no token is provided but secret exists', async () => {
    mockConfig.get.mockImplementation((key: string) => (key === 'RECAPTCHA_SECRET' ? 'secret' : undefined));
    const svc = new RecaptchaService(mockConfig as any);
    const ok = await svc.verify(null);
  ...
  it('validates score and action - passes when above minScore', async () => {
    ...
    (globalThis.fetch as jest.Mock).mockResolvedValueOnce({ ok: true, json: async () => ({ success: true, score: 0.8, action: 'admin_signup' }) });
    const svc = new RecaptchaService(mockConfig as any);
  ...
  it('fails when score below threshold', async () => {
    mockConfig.get.mockImplementation((key: string) => (key === 'RECAPTCHA_SECRET' ? 'secret' : undefined));
    (globalThis.fetch as jest.Mock).mockResolvedValueOnce({ ok: true, json: async () => ({ success: true, score: 0.3, action: 'admin_signup' }) });
    const svc = new RecaptchaService(mockConfig as any);
    const ok = await svc.verify('token', 'admin_signup');
    expect(ok).toBe(false);
  });
