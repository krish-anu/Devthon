...
export default function BookingHistoryPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const totalEarned = bookings.reduce((sum, b) => sum + (b.finalAmountLkr ?? 0), 0);
    const totalWeight = bookings.reduce((sum, b) => sum + (b.actualWeightKg ?? 0), 0);
  ...
  const exportCsv = () => {
    ...
    const header = ['Booking ID', 'Waste Type', 'Weight', 'Amount', 'Status', 'Date'];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <KpiCard label="Total Earned" value={`LKR ${metrics.totalEarned.toFixed(0)}`} />
        <KpiCard label="Total Weight" value={`${metrics.totalWeight.toFixed(1)} kg`} />
      </div>
          ...
          <TableBody>
            {bookings.map((booking) => (
              <TableRow key={booking.id}>
                <TableCell>
                  <a href={`/app/bookings/${booking.id}`} className="text-emerald-200">
