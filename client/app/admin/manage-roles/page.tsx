...
export default function ManageRolesPage() {
  ...
  const changeRoleMutation = useMutation({
    ...
    onError: (err: any) => {
      toast({
        title: "Update failed",
        description: err?.message ?? "Unable to change role",
        variant: "error",
      });
    },
  });
      ...
      <Card className="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-(--muted)">
            View all users and change roles (Super Admin only).
          </p>
        </div>
        <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center lg:w-auto">
          ...
          <label className="text-sm text-(--muted) sm:whitespace-nowrap">
            <input
              type="checkbox"
              className="mr-2"
              checked={nonCustomerOnly}
              onChange={(e) => setNonCustomerOnly(e.target.checked)}
            />
        ...
        {isLoading ? (
          ...
          <>
                  ...
                  <TableRow key={u.id}>
                    ...
                    <TableCell>
                      <select
                        ...
                        onChange={(e) =>
                          setEditing((s) => ({ ...s, [u.id]: e.target.value }))
                        }
            ...
            <Pagination
              ...
              onNext={() => {
                setAfterCursor((data as any)?.nextCursor ?? null);
                setBeforeCursor(null);
              }}
              onPrev={() => {
                setBeforeCursor((data as any)?.prevCursor ?? null);
                setAfterCursor(null);
              }}
              limit={limit}
              onLimitChange={(n) => {
